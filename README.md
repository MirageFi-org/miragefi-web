...
- **Writes and private reads** (profile, attestations) are sent to `POST /api/platform/db` with the user's Privy access token. The route verifies the token, validates the arguments in `src/lib/platform/ops.ts`, and runs the operation with a service-role Supabase client that carries the verified user id in an `x-miragefi-actor` header. The database function `app_user_id()` trusts that header only on service-role requests.
