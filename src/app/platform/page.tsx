    ...
    <div className="space-y-8">
              ...
              {markets.map((m) => {
                ...
                return (
                  <tr key={m.symbol} className="transition-colors hover:bg-mf-cloud/50">
                    ...
                    <Td className="font-mono text-mf-teal">{fmtBps(q.halfSpreadBps)}</Td>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <Link key={c.href} href={c.href} className="sheet group flex items-start justify-between gap-4 p-5 transition-colors hover:border-mf-teal">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-mf-teal">MF · {c.code}</p>
              <h3 className="mt-1.5 font-heading text-lg font-medium text-mf-ink">{c.title}</h3>
              <p className="mt-1 text-[13px] leading-relaxed text-mf-muted">{c.body}</p>
            </div>
            <ArrowRight className="mt-1 size-4 shrink-0 text-mf-faint transition-transform group-hover:translate-x-1 group-hover:text-mf-teal" aria-hidden="true" />
          </Link>
        ))}
      </div>
    </div>
