  ...
  return (
    <div className="space-y-8">
      ...
      >
          ...
          <Table minWidth={760}>
            ...
            <tbody>
              {markets.map((m) => {
                ...
                return (
                  <tr key={m.symbol} className="transition-colors hover:bg-mf-cloud/50">
                    <Td>
                      <div className="font-mono font-medium text-mf-ink">{m.symbol}</div>
                      <div className="text-[12px] text-mf-faint">{m.name}</div>
                    </Td>
                    <Td>
                    ...
                    <Td>
                      <div className="flex items-center gap-2">
                        <div className="relative h-2 w-28 overflow-hidden bg-mf-mist">
                          {/* Band: 30% to 70% of value */}
                          <div className="absolute inset-y-0 left-[30%] right-[30%] bg-mf-teal/15" />
                          <div
                            className={cn(
                              "absolute inset-y-0 left-0",
                              off > 1500 ? "bg-mf-rose/70" : "bg-mf-teal/80"
                            )}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                        <span className="font-mono text-[12px] text-mf-slate">{pct.toFixed(1)}%</span>
                      </div>
                    </Td>
                    <Td className="font-mono">{fmtBps(tier.baseHalfSpreadBps)}</Td>
                    <Td className="font-mono text-mf-teal">{100 - FEES.spreadShareBps / 100}%</Td>
                    <Td>
                      {authenticated ? (
        ...
        <p className="mt-4 font-mono text-[10.5px] uppercase tracking-[0.14em] text-mf-faint">
          Deposits value at the live mid, so they pause while a market is halted; withdrawals never do.
        </p>
      </Sheet>

      <div className="sheet grid divide-y divide-mf-line overflow-hidden md:grid-cols-3 md:divide-x md:divide-y-0">
        {[
          {
            label: "What you earn",
          ...
          <div key={c.label} className="p-6">
            <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-mf-teal">{c.label}</p>
            <p className="mt-3 text-[14px] leading-relaxed text-mf-slate">{c.body}</p>
          </div>
        ))}
      </div>
    </div>
