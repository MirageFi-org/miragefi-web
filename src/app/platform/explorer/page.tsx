    ...
    <div className="space-y-8">
      ...
      >
        {venue.loading && !venue.data ? (
          ...
          <Table minWidth={680}>
            ...
            <tbody>
              {fills.map((f, i) => (
                <tr key={i} className="transition-colors hover:bg-mf-cloud/50">
                  <Td className="font-mono text-mf-faint">{fmtAgo(f.minutesAgo)}</Td>
                  <Td className="font-mono font-medium text-mf-ink">{f.symbol}</Td>
                  <Td>
                  ...
                  <Td className="text-right font-mono text-mf-teal">{fmtBps(f.executionBps)}</Td>
                </tr>
              ))}
            </tbody>
          </Table>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Sheet label="Parameters · in force now" meta="timelocked, rationale published">
          ...
          <p className="mt-3 font-mono text-[10.5px] uppercase tracking-[0.14em] text-mf-faint">
        ...
        <Sheet label="Governance log · every change, before it bites" meta="no fund access, ever">
          <ol className="space-y-4">
            {governance.map((entry) => (
              <li key={entry.date + entry.action} className="border-l-2 border-mf-teal/40 pl-4">
                <div className="flex items-baseline justify-between gap-3">
                  <span className="text-[14px] font-medium text-mf-ink">{entry.action}</span>
                  <span className="font-mono text-[11px] text-mf-faint">{entry.date}</span>
                </div>
                <p className="mt-1 text-[13px] leading-relaxed text-mf-muted">{entry.detail}</p>
              </li>
            ))}
          </ol>
        </Sheet>
