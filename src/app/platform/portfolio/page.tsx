  ...
  return (
    <div className="space-y-6">
        ...
        {fills.loading && !fills.data ? (
          ...
          <>
            <Table minWidth={680}>
              ...
              <tbody>
                {fills.data.map((f) => (
                  <tr key={f.id} className="transition-colors hover:bg-mf-cloud/50">
                    <Td className="font-mono text-mf-faint">
                      {fmtAgo(Math.max(0, (now - new Date(f.executed_at).getTime()) / 60_000))}
                    </Td>
                    <Td className="font-mono font-medium text-mf-ink">{f.markets?.symbol ?? "?"}</Td>
                    <Td>
                    ...
                    <Td className="text-right font-mono text-mf-teal">{fmtBps(f.half_spread_bps + f.fee_bps)}</Td>
                  </tr>
                ))}
              </tbody>
            </Table>
      ...
      <Sheet label="Eligibility · role attestations" meta="TRADER · LP · MAKER · RELAYER">
          ...
          <div className="flex flex-wrap gap-2">
            {ROLES.map((role) => {
              ...
              return (
                <div
                  key={role}
                  className="flex items-center gap-2.5 border border-mf-line bg-mf-paper px-3 py-2"
                >
                  <span className="font-mono text-[12px] text-mf-ink">{role}</span>
        ...
        <p className="mt-4 text-[13px] leading-relaxed text-mf-muted">
