  ...
  if (venue.loading && !venue.data) {
    return (
      <div className="space-y-8">
        <PageHead code="MF · 02 · Swap" title="The quote is the invoice" />
        <Skeleton rows={8} />
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="sheet overflow-hidden">
        <div className="sheet-head">
          <span>Ticket · {market.symbol} / {QUOTE_SYMBOL}</span>
        ...
        <div className="grid lg:grid-cols-[1fr_300px]">
          ...
          <div className="p-5 md:p-7">
            <div className="grid gap-5 sm:grid-cols-[1fr_auto_1fr] sm:items-end">
              ...
              <button
                ...
                className="cursor-pointer mx-auto mb-1 inline-flex size-10 items-center justify-center border border-mf-line-strong bg-mf-paper text-mf-teal transition-colors hover:border-mf-teal"
              >
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <p className="text-[12px] leading-relaxed text-mf-faint">
                {authenticated
          ...
          {/* The stub */}
          <aside className="border-t-2 border-dashed border-mf-line-strong bg-mf-cloud/50 p-5 md:p-6 lg:border-l-2 lg:border-t-0">
            <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-mf-faint">Stub · route details</p>
            ...
            <dl className="mt-6 space-y-2.5 font-mono text-[12px]">
              <div className="flex justify-between">
                <dt className="text-mf-muted">Route</dt>
                <dd className="text-mf-ink">{buyToken ? `${QUOTE_SYMBOL} → ${market.symbol}` : `${market.symbol} → ${QUOTE_SYMBOL}`}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-mf-muted">Venue</dt>
                <dd className="text-mf-ink">vault, RFQ if better</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-mf-muted">Clip this session</dt>
                <dd className="text-mf-ink">{fmtMoney(quote.clip, QUOTE_SYMBOL, 0)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-mf-muted">Oracle band</dt>
                <dd className="text-mf-ink">{fmtBps(tier.oracleBandBps, 0)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-mf-muted">RFQ fee</dt>
                <dd className="text-mf-ink">{fmtBps(FEES.rfqFeeBps)}</dd>
              </div>
            </dl>

            <p className="mt-6 border-t border-mf-line pt-4 font-mono text-[10px] uppercase tracking-[0.16em] text-mf-faint">
              MF-4663 · atomic settlement · no custody
            </p>
          </aside>
        </div>
      </div>
